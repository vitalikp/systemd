...
static ShowStatus arg_show_status = SHOW_STATUS_YES;
        ...
        if (getpid() != 1)
                ...
                if (pid < 0)
                        ...
                        if (r < 0)
                                ...
                                log_error("Caught <%s>, core dump failed (child "PID_FMT", code=%s, status=%i/%s).",
...
static void install_crash_handler(void) {
        ...
        if (r < 0)
                log_debug("I had trouble setting up the crash handler, ignoring: %s", strerror(-r));
}
